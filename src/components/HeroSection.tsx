    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 md:px-12 py-3 md:py-4 bg-black/65 backdrop-blur-[8px]" dir="rtl">
        ...
        <div className="hidden md:flex flex-1 items-center justify-center gap-10 lg:gap-14 text-lg font-semibold font-body text-[#D4AF37] mx-8">
          {navLinks.map((link) => (
            <a key={link.href} href={link.href} className="relative hover:text-[#FFD700] transition-colors duration-300 after:content-[''] after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-[2px] after:bg-[#D4AF37] after:transition-all after:duration-300 hover:after:w-full hover:drop-shadow-[0_0_6px_rgba(212,175,55,0.5)]">{link.label}</a>
          ))}
        </div>
        ...
        <button className="md:hidden text-[#D4AF37] p-2" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} aria-label="Toggle menu">
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-40 bg-black/90 backdrop-blur-md flex flex-col items-center justify-center gap-8" dir="rtl">
          {navLinks.map((link) => (
            <a key={link.href} href={link.href} className="text-[#D4AF37] text-2xl font-body hover:text-[#FFD700] transition-colors tracking-wider" onClick={() => setMobileMenuOpen(false)}>{link.label}</a>
          ))}
          <a href="#contact" className="text-[#FFD700] text-xl border border-[#FFD700] px-8 py-3 rounded-sm hover:bg-[#D4AF37]/10 transition-colors mt-4" onClick={() => setMobileMenuOpen(false)}>تواصل معنا</a>
        </div>
      )}
            ...
            {hasLine2 ? (
              <>
                <span className="hero-text-white block text-3xl md:text-5xl lg:text-7xl tracking-[0.15em] min-h-[1.2em]" style={{ color: '#FFFFFF' }}>
                  {displayLine1}
                  {(phase === "typing1" || phase === "deleting1") && <span className="inline-block w-[3px] h-[0.8em] bg-[#D4AF37] ml-1 animate-pulse align-baseline" />}
                </span>
                <span className="hero-text-gold block text-3xl md:text-5xl lg:text-7xl tracking-[0.15em] min-h-[1.2em]" style={{ color: '#D4AF37' }}>
                  {displayLine2}
                  {(phase === "typing2" || phase === "deleting2") && <span className="inline-block w-[3px] h-[0.8em] bg-[#D4AF37] ml-1 animate-pulse align-baseline" />}
                </span>
              </>
            ) : (
              <span className="block text-4xl md:text-6xl lg:text-8xl tracking-[0.08em] min-h-[2.4em] flex items-center justify-center">
                ...
                {(phase === "typing1" || phase === "deleting1") && <span className="inline-block w-[3px] h-[0.8em] bg-[#D4AF37] mr-1 animate-pulse align-baseline" />}
              </span>
            )}
          </h1>
