    ...
    <section className="py-24 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-5xl font-bold text-gold-gradient mb-4">احجز وحدتك الآن</h2>
          <div className="section-divider w-24 mx-auto mb-6" />
          <p className="text-muted-foreground text-lg max-w-xl mx-auto">تواصل معنا لحجز وحدتك التجارية في The View Avenue</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-card border border-primary/30 rounded-2xl p-8 md:p-12 glow-gold space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Input id="name" placeholder="أدخل اسمك الكامل" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} maxLength={100} className="bg-secondary border-border focus:border-primary" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="phone" className="text-foreground">رقم الجوال *</Label>
              <Input id="phone" type="tel" placeholder="05XXXXXXXX" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} maxLength={15} className="bg-secondary border-border focus:border-primary" dir="ltr" />
            </div>
          </div>
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-3">
              {businessTypes.map((type) => (
                <button key={type} type="button" onClick={() => setForm({ ...form, business: type })} className={`px-4 py-2 rounded-full border text-sm transition-all ${form.business === type ? "bg-primary text-primary-foreground border-primary" : "bg-secondary text-foreground border-border hover:border-primary/50"}`}>{type}</button>
              ))}
            </div>
          </div>
          <div className="space-y-2">
            <Label htmlFor="message" className="text-foreground">رسالتك</Label>
            <Textarea id="message" placeholder="اكتب رسالتك أو استفسارك هنا..." value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} maxLength={1000} className="bg-secondary border-border focus:border-primary min-h-[120px]" />
          </div>
          <Button type="submit" disabled={sending} className="w-full bg-gold-gradient text-primary-foreground font-bold text-lg py-6 rounded-xl hover:opacity-90 transition-opacity">
            <Send className="w-5 h-5 ml-2" /> إرسال عبر واتساب
          </Button>
        </form>
      </div>
    </section>
