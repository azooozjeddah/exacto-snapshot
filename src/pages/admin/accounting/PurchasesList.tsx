...
interface Purchase {
  id: number; purchase_number: string; purchase_date: string; total: number; status: string;
...
export default function PurchasesList() {
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from('purchases').insert({
      purchase_number: form.purchase_number, supplier_id: form.supplier_id ? parseInt(form.supplier_id) : null,
      total, subtotal: total, tax_amount: 0, status: form.status as any,
    });
  ...
  const handleExport = () => {
    exportToCSV(
      ['الرقم', 'التاريخ', 'المورد', 'المبلغ', 'الحالة'],
      filtered.map(p => [p.purchase_number, p.purchase_date, (p.suppliers as any)?.name_ar || '', Number(p.total), statusMap[p.status]?.ar || p.status]),
      'purchases'
    );
  };
    ...
    <div dir="rtl">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogTrigger asChild><Button className="bg-[#D4AF37] hover:bg-[#b8962e] text-white"><Plus className="h-4 w-4 ml-2" /> مشترى جديد</Button></DialogTrigger>
          <DialogContent className="sm:max-w-md" dir="rtl">
            ...
            <div className="space-y-4 mt-4">
              ...
              <div><Label>المورد</Label><Select value={form.supplier_id} onValueChange={v => setForm({ ...form, supplier_id: v })}><SelectTrigger><SelectValue placeholder="اختر المورد" /></SelectTrigger><SelectContent>{suppliers.map(s => <SelectItem key={s.id} value={String(s.id)}>{s.name_ar}</SelectItem>)}</SelectContent></Select></div>
              <div><Label>المبلغ</Label><Input type="number" value={form.total} onChange={e => setForm({ ...form, total: e.target.value })} dir="ltr" /></div>
              <div><Label>الحالة</Label><Select value={form.status} onValueChange={v => setForm({ ...form, status: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.entries(statusMap).map(([k, v]) => <SelectItem key={k} value={k}>{v.ar}</SelectItem>)}</SelectContent></Select></div>
      ...
      <TableToolbar searchValue={search} onSearchChange={setSearch} searchPlaceholder="بحث برقم المشترى أو المورد..."
        onExportCSV={handleExport} helpText="سجّل وأدر مشترياتك هنا." helpTextEn="Record and manage your purchases." />

      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <Table>
          <TableHeader><TableRow className="bg-gray-50/80">
            <TableHead className="text-right font-bold">الرقم</TableHead><TableHead className="text-right font-bold">التاريخ</TableHead>
            <TableHead className="text-right font-bold">المورد</TableHead><TableHead className="text-right font-bold">المبلغ</TableHead>
            <TableHead className="text-right font-bold">الحالة</TableHead><TableHead className="text-right font-bold">إجراءات</TableHead>
              ...
              <TableRow key={p.id} className="hover:bg-gray-50/50 transition-colors">
                ...
                <TableCell>
                  <AlertDialog>
                    ...
                    <AlertDialogContent dir="rtl"><AlertDialogHeader><AlertDialogTitle>تأكيد الحذف</AlertDialogTitle><AlertDialogDescription>هل أنت متأكد من حذف المشترى "{p.purchase_number}"؟</AlertDialogDescription></AlertDialogHeader>
      ...
      <p className="text-xs text-gray-400 mt-2 text-center">عدد المشتريات: {filtered.length} | الإجمالي: {filtered.reduce((s, p) => s + Number(p.total), 0).toLocaleString()} ر.س</p>
    </div>
  );
}
