...
import { Menu, X, LogOut, ExternalLink, BarChart3, Paperclip, Shield, LayoutDashboard } from 'lucide-react';

const navItems = [
  { label: 'لوحة المحاسب', labelEn: 'Dashboard', icon: LayoutDashboard, to: '/accountant', end: true },
  { label: 'التقارير المالية', labelEn: 'Reports', icon: BarChart3, to: '/accountant/reports' },
  { label: 'المستندات', labelEn: 'Attachments', icon: Paperclip, to: '/accountant/attachments' },
  { label: 'سجل التدقيق', labelEn: 'Audit Trail', icon: Shield, to: '/accountant/audit' },
];
    ...
    <div dir="rtl" className="min-h-screen flex bg-[#F8F9FA]" style={{ fontFamily: "'Tajawal', sans-serif" }}>
      ...
      <aside className={`fixed lg:static top-0 right-0 z-40 h-screen lg:h-auto w-[260px] min-h-screen bg-white border-l border-gray-200 flex flex-col shrink-0 transition-transform duration-300 ${collapsed ? 'translate-x-full lg:translate-x-0 lg:w-0 lg:overflow-hidden lg:border-0' : 'translate-x-0'}`}>
        <div className="p-6 border-b border-gray-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">لوحة المحاسب</p>
          </div>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-14 bg-white border-b border-gray-200 flex items-center px-4 lg:px-6 shrink-0">
          ...
          <span className="text-sm font-medium text-gray-700">لوحة المحاسب - Accountant Panel</span>
        </header>
